...
Meet Mycel, enter your real-world baseline, then select Verdant Reach from the spinning world map. Move with WASD, look with the mouse, jump with Space, dash with Shift, switch between first and third person with V, punch with left click, interact with E, open the journal with J, open the map with M, and pause with Escape. Find three memories along branching trails and return to the Canopy Gate. Progress saves in this browser.

This is a prototype biome, not an open-world production release. The shellbacks and thornlings are 3D animated creatures; defeating them does not award XP. Real-world activities logged under Weekly Quest grant XP; baseline metrics shape your character stats. The other three world-map realms are level gated previews, not playable yet. The leaderboard is not connected to a shared service. The 3D profile has its own local save and does not read or modify the 2D game's save.
